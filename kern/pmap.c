...
page_init(void)
{
	...
	// free pages!
	size_t i;
	int busy = ((uint32_t)boot_alloc(0)-KERNBASE)/PGSIZE;//to get the busy page
	int io_hole = 96; //the number of busy page in io hole area.
	for (i = 0; i < npages; i++) {
		if(i==0){
			pages[i].pp_ref = 1;
		}
		else if(i >= npages_basemem && i < npages_basemem + io_hole + busy)
		{
			pages[i].pp_ref = 1;
		}
		else{
...
page_alloc(int alloc_flags)
{
   ...
   if(page_free_list == NULL) //if there is no more pages can be used. 
        return NULL;  
  
    struct PageInfo* page = page_free_list;  
    page_free_list = page->pp_link;
    page->pp_link = 0;
    if(alloc_flags & ALLOC_ZERO)  // fill the entire.
